...
async function signApplication (opts: ValidatedSignOptions, identity: Identity) {
  ...
  /**
   * For each level in the file tree, group together all files that use the same
   * signing arguments so we can sign all of them with a single call to
   * `codesign`, while still ensuring that the app files are signed from the
   * inside out.
   */
  ...
  for (const filePath of [...children, opts.app]) {
    ...
    const filesWithSameArgsMap = filesWithSameArgsByDepth.get(fileDepth) ?? new Map<string, string[]>();

    const fileWithSameArgsMapKey = JSON.stringify(perFileArgs);
    filesWithSameArgsMap.set(fileWithSameArgsMapKey, (filesWithSameArgsMap.get(fileWithSameArgsMapKey) ?? ([] as string[])).concat(filePath));
